...
__global__ void MatrixMulKernel(int *d_M, int *d_N, int *d_P, int m, int n, int p)
{	
...
__global__ void MatrixMulKernelTiles(int *d_MatA, int *d_MatB, int *d_MatR, int m, int n, int o)
{	
	// ds => data shared
...
// Funcion que llama Multiplicacion Kernel sin Tiles
void matrixMulTiles(int *A, int *B, int *C, int m, int n, int p)
{
	int *d_A, *d_B, *d_C;
	//Reservo Memoria en el dispositivo
	cudaMalloc((void **)&d_A,m*n*sizeof(int));
...
void multMatrixsequential (int *h_MatA, int *h_MatB, int *h_MatR, int m, int n, int o)
{
	clock_t t5;
	t5 = clock();
...
void agregarDatos(int *A, int m, int n)
{
	for(int i = 0; i < m*n; i++)
	{
		A[i] = rand() % 3;
...
// IMPRIMIR MATRIZ
void imprimirMatriz (int *A, int a, int b)
{
    for (int i = 0; i < b; i++)
    {
		for (int j = 0; j < a; j++)
...
int compararMatrices(int *MA, int *MB, int m, int n)
{
	int tam = m*n;
	for (int i = 0; i < tam; ++i)
	{
...
int main()
{
	...
	RS = (int*)malloc(m*p*sizeof(int)); 
	RP = (int*)malloc(m*p*sizeof(int));
	RPT = (int*)malloc(m*p*sizeof(int));
	...
	printf("\n...Matriz B...\n");
	imprimirMatriz (B, p, n);
	*/
	multMatrixsequential(A, B, RS, m, n, p);
	//imprimirMatriz(RS, p, m);

	matrixMul(A, B, RP, m, n, p);
	//imprimirMatriz(RP, p, m);

	matrixMulTiles(A, B, RPT, m, n, p);
	//imprimirMatriz(RPT, p, m);
